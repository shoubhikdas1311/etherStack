...
extern signed char fnEthernetEvent(unsigned char *ucEvent, ETHERNET_FRAME *rx_frame)
{
    ...
    if (EMAC_RX_INTERRUPT == *ucEvent) {
        ...
        else if (ptrRxBd->usBDLength == 0) {
          //_TOGGLE_PORT(D, PORTD_BIT3);
            return -1;
        }
...
static __interrupt void _fec_rx_frame_isr(void)
{
    ...
    if (ptrMagicRxBd != 0) {
        if ((ptrMagicRxBd->usBDControl & EMPTY_BUFFER) == 0) {           // ignore if buffer is empty
            if (!(ptrMagicRxBd->usBDControl & (TRUNCATED_FRAME | OVERRUN_FRAME))) { // only handle valid frames
                unsigned short usLength = fnLE_ENET_word(ptrMagicRxBd->usBDLength); // length in the buffer
                if (!(RCR & CRCFWD)) {                                   // if the CRC-32 is not being removed reduce the reception length (the user knows that it will be in the buffer after the data)
                    usLength -= 4;
                }
...
_not_magic_frame:
            if (ptrMagicRxBd->usBDControl & WRAP_BIT_RX) {
...

{
    ...
    if (PHY_LINK_STATE_CHANGE & usInterrupt) {                           // a link state change has taken place
        ...
        if (((iFullDuplex != 0) && (RCR & DRT)) || ((iFullDuplex == 0) && (TCR & FDEN))) { // only restart if duplex mode has changed
...
#endif
    while (!(EIR & MII)) {                                               // wait until the write has completed
#if defined _WINDOWS
        EIR |= MII;
#endif
    };
...
extern int fnConfigEthernet(ETHTABLE *pars)
{
    ...
    #else
    if (pars->usMode & PASS_CRC32_RX) {                                  // {13}
  ...
  //ENET_RACC = (RACC_LINEDIS);                                          // automatically discard reception frames with MAC errors
    ...
    if (usRxSize & 0x0f) {                                               // ensure size is suitable (divisible by 16)
        usRxSize &= ~0x0f;
        usRxSize += 0x10;
    }
    #if defined USE_MULTIPLE_BUFFERS
    ...
    if (pars->usSizeTx & 0x07) {                                         // ensure size is suitable (divisible by 8)
        pars->usSizeTx &= ~0x07;
    ...
    if (!(pars->usMode & (LAN_10M | LAN_100M))) {                        // when in auto-negotiation mode
...
extern void fnFreeEthernetBuffer(int iBufNr)
{
    ...
    #endif
    if (ptrRxBd->usBDControl & WRAP_BIT_RX) {
...
extern QUEUE_TRANSFER fnStartEthTx(QUEUE_TRANSFER DataLen, unsigned char *ptr_put)
{
    ...
    if (ptrTxBd->usBDControl & WRAP_BIT_TX) {                            // prepare local pointer for next write
